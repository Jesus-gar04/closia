...
export function OutfitCard({ outfit }) {
  ...
  const probarLook = () => {
    ...
    if (prendas.length < ids.length) {
      toast('Algunas prendas ya no están disponibles', {
        icon: '⚠️'
      })
    }
      ...
      >
        {outfit.screenshot_url ? (
          ...
          <div className="w-full h-full flex items-center justify-center">
            <div className="w-16 h-16 rounded-full bg-accent-soft flex items-center justify-center text-accent-deep text-[22px]">
              ★
            </div>
          </div>
        )}
        ...
        >
          <button
            ...
            className="mb-3 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-surface/95 text-ink text-[12px] font-medium shadow-card"
          >
            <svg
              width="13"
              height="13"
        ...
        <div
          className={clsx(
            'absolute top-2.5 right-2.5 flex gap-1.5 transition-opacity',
            mostrarAcciones
              ? 'opacity-100'
              : 'opacity-0 group-hover:opacity-100'
          )}
          ...
          {[1, 2, 3, 4, 5].map((e) => (
            <button
              ...
              className={clsx(
                ...
                e <= outfit.rating
                  ? 'text-accent'
                  : 'text-hairline hover:text-accent/60'
              )}
            >
              ★
            </button>
        ...
        {outfit.tags?.length > 0 && (
          <div className="flex flex-wrap gap-1">
            {outfit.tags.slice(0, 3).map((tag) => (
              <span
                key={tag}
                className="px-2 py-0.5 bg-tinted text-muted text-[10.5px] rounded-full"
              >
                {tag}
              </span>
            ))}
      ...
      {confirmar && (
        <div className="px-3.5 pb-3.5 flex gap-2">
          <button
            ...
            className="flex-1 py-1.5 bg-danger text-canvas text-[11.5px] rounded-sm font-medium"
          >
            Eliminar
          </button>
          ...
          <button
            ...
            className="flex-1 py-1.5 bg-tinted hover:bg-deep text-ink-soft text-[11.5px] rounded-sm"
          >
            Cancelar
          </button>
        </div>
